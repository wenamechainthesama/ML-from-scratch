...
class RandomizedSearchCV:
    def __init__(
        self, estimator, params_grid: dict, cv: int, evaluation_function, num_choices
    ):
        """
    ...
    def fit(self, X, y):
        # Get all possible combinations of params and select some randomly
        all_params_combos = list(product(*self.params_grid.values()))
        params_idxs = np.random.choice(len(all_params_combos), self.num_choices)
        random_params_combos = [
            tuple(params)
            for params_idx, params in enumerate(all_params_combos)
            if params_idx in params_idxs
        ]
        ...
        for params in random_params_combos:
            # Create model
